...
import { addDays, eachDayOfInterval, isSameDay, isWithinInterval, format, differenceInDays } from "date-fns"
import { getBookedDates, addBooking } from "@/utils/availability"
...
export function useBookingCalendar({ roomId, basePrice, minNights = 1, maxNights = 30 }: UseBookingCalendarProps) {
  ...
  // Load booked dates
  useEffect(() => {
    setBookedDates(getBookedDates(roomId))
  }, [roomId])
  ...
  const submitBooking = useCallback(async () => {
    ...
    try {
      ...
      // Add the booked dates to our local state
      if (booking.checkIn && booking.checkOut) {
        addBooking(roomId, booking.checkIn, booking.checkOut)
